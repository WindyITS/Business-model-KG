...
class AnalystPipelineRunnerTests(unittest.TestCase):
    ...
    def test_analyst_runner_builds_memo_then_graph_then_critique(self):
        ...
        self.assertIn("for each [Segment] block and each [Offering] block, read the entire block to avoid missing information", captured_messages[2][1]["content"])
        ...
        self.assertIn("for each [Segment] block and each [Offering] block, read the entire block and consider all subsections before emitting triples", captured_messages[2][0]["content"])
