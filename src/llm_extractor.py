    ...
    def extract_canonical_pipeline(
        ...
        *,
        ...
        pass4_corporate_prompt = (
            ...
            "- for OPERATES_IN, be willing to capture all named countries or approved macro-regions where the company clearly operates, conducts business, or has meaningful market presence\n"
            "- read geography evidence across the full filing, not just one sentence or one section\n"
            "- a country can qualify for OPERATES_IN when the filing gives any credible company-level presence signal, including a named subsidiary or local entity, employee presence, labor structure such as a works council, explicit customer or revenue presence, country-specific platform availability, repeated country-specific operating examples, or other direct evidence that the company does business there in a meaningful way\n"
            "- do not require headquarters, offices, or incorporation to be stated if other filing evidence clearly shows meaningful market presence\n"
            "- if multiple different parts of the filing point to the same country, treat that as reinforcing support for OPERATES_IN\n"
            "- prefer recall over unnecessary omission when a named geography is clearly tied to the company's own business presence\n"
            "- include United States and other named countries when the filing describes company operations, employees, customers, partnerships through local entities, commercial availability, or meaningful revenue concentration there\n"
            "- exclude vague global labels such as abroad, worldwide, around the world, select countries, international markets, or other non-normalized placeholders\n"
            "- exclude cities, office sites, and facility names\n"
            "- exclude a geography when it appears only as a customer anecdote, user example, partner location, regulatory reference, patent or trademark jurisdiction, or other incidental mention that does not show the company's own business presence there\n"
            "- do not add a country solely because a partner is based there unless the filing also ties the company itself to that country through a subsidiary, local entity, employees, customers, commercial activity, or other company-level presence\n"
            "- when uncertain, ask whether the filing presents that geography as part of the company's real operating footprint rather than as background context\n"
            "</pass_specific_focus>\n\n"
            "<format_reminder>\n"
            'Return ONLY a raw JSON object containing "extraction_notes" and the "triples" array.\n'
            "Do not use markdown code blocks (```json).\n"
